...
async function weather(message) {
  ...
  if (!message.args.length) {
    ...
    message.output = '**' + weatherData[0].location.name + ' right now:**\n' + current.temperature + '\xB0C/' + (current.temperature * 1.8 + 32) + '\xB0F ' + current.skytext + ' feels like ' + current.feelslike + '\xB0C/' + (current.feelslike * 1.8 + 32) + '\xB0F ' + current.winddisplay + ' wind\n**Forecast for tomorrow:**\nHigh: ' + forecast[1].high + '\xB0C/' + (forecast[1].high * 1.8 + 32) + '\xB0F, low: ' + forecast[1].low + '\xB0C/' + (forecast[1].low * 1.8 + 32) + '\xB0F ' + forecast[1].skytextday + ' with ' + forecast[1].precip + '% chance precip.';
