...
export default async function covid19Cmd(message, bot) {
  ...
  if (message.command.args.length <= 0) {
    ...
    if (location.country) message.output.addField("Critical",          location.critical.toLocaleString(),           true)
    ...
    if (location.country) message.output.addField("Cases per million", location.casesPerOneMillion.toLocaleString(), true)
